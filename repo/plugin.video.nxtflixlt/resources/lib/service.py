...
pause_services_prop = 'nxtflixlt.pause_services'
firstrun_update_prop = 'nxtflixlt.firstrun_update'
current_skin_prop = 'nxtflixlt.current_skin'
...
class SetAddonConstants:
	def run(self):
		kodi_utils.logger('NXTFlix Light', 'SetAddonConstants Service Starting')
		import random
		addon_items = [
			('nxtflixlt.playback_key', str(random.randint(1000, 10000))),
			('nxtflixlt.addon_version', kodi_utils.addon_info('version')),
			('nxtflixlt.addon_path', kodi_utils.addon_info('path')),
			('nxtflixlt.addon_profile', kodi_utils.translate_path(kodi_utils.addon_info('profile'))),
			('nxtflixlt.addon_icon', kodi_utils.translate_path(kodi_utils.addon_info('icon'))),
			('nxtflixlt.addon_icon_mini', os.path.join(kodi_utils.addon_info('path'), 'resources', 'media', 'addon_icons', 'minis',
			os.path.basename(kodi_utils.translate_path(kodi_utils.addon_info('icon'))))),
			('nxtflixlt.addon_fanart', kodi_utils.addon_fanart())
					]
		for item in addon_items: kodi_utils.set_property(*item)
		return kodi_utils.logger('NXTFlix Light', 'SetAddonConstants Service Finished')

class DatabaseMaintenance:
	def run(self):
		kodi_utils.logger('NXTFlix Light', 'DatabaseMaintenance Service Starting')
		from caches.base_cache import check_databases_integrity
		check_databases_integrity(silent=True)
		return kodi_utils.logger('NXTFlix Light', 'DatabaseMaintenance Service Finished')

class SyncSettings:
	def run(self):
		kodi_utils.logger('NXTFlix Light', 'SyncSettings Service Starting')
		sync_settings()
		return kodi_utils.logger('NXTFlix Light', 'SyncSettings Service Finished')

class OnUpdateChanges:
	def run(self):
		kodi_utils.logger('NXTFlix Light', 'OnUpdateChanges Service Starting')
		try:
			for method in list(filter(lambda x: x[0] != 'run', inspect.getmembers(OnUpdateChanges, predicate=inspect.isfunction))):
				if not get_setting('nxtflixlt.updatechecks.%s' % method[0], 'false') == 'true':
					method[1](self)
					set_setting('updatechecks.%s' % method[0], 'true')
		except: pass
		return kodi_utils.logger('NXTFlix Light', 'OnUpdateChanges Service Finished')
	...
	def refresh_addon_keys(self):
		...
		show_dialog = False
		if get_setting('nxtflixlt.tmdb_api') == 'b370b60447737762ca38457bd77579b3':
			...
			show_dialog = True
		if get_setting('nxtflixlt.trakt.client') == '1038ef327e86e7f6d39d80d2eb5479bff66dd8394e813c5e0e387af0f84d89fb':
			...
			show_dialog = True
		if show_dialog:
			text = 'The original developer of NXTFlix Light has revoked all keys (Trakt, TMDb) that have been used up until now '\
			'within this addon, with permission, due to security concerns.'
			kodi_utils.ok_dialog(heading='Addon Credentials Reset', text=text)
			text = 'This unfortunately means you will need to re-authenticate your Trakt & TMDb accounts through NXTFlix Light AM  if you are currently using them. ' \
			'My apologies, but with the previous keys being revoked, this is necessary.'
			kodi_utils.ok_dialog(heading='Addon Credentials Reset', text=text)

class CustomWindowsPrepare:
	def run(self):
		kodi_utils.logger('NXTFlix Light', 'CustomWindowsPrepare Service Starting')
		...
		return kodi_utils.logger('NXTFlix Light', 'CustomWindowsPrepare Service Finished')

class TraktMonitor:
	def run(self):
		kodi_utils.logger('NXTFlix Light', 'TraktMonitor Service Starting')
		...
		while not monitor.abortRequested():
			...
			try:
				...
				if status == 'failed': kodi_utils.logger('NXTFlix Light', trakt_service_string % ('Failed. Error from Trakt', next_update_string))
				elif status == 'no_auth': kodi_utils.logger('NXTFlix Light', trakt_service_string % ('Not Run. No Current Trakt Account', next_update_string))
				else:
					if status in ('success', 'no account'):
						kodi_utils.logger('NXTFlix Light', trakt_service_string % ('Success. %s' % trakt_success_line_dict[status], next_update_string))
					else:
						kodi_utils.logger('NXTFlix Light', trakt_service_string % ('Success. No Changes Needed', next_update_string))# 'not needed'
					if status == 'success' and get_setting('nxtflixlt.trakt.refresh_widgets', 'false') == 'true':
						kodi_utils.run_plugin({'mode': 'kodi_refresh'})
			except Exception as e: kodi_utils.logger('NXTFlix Light', trakt_service_string % ('Failed', 'The following Error Occured: %s' % str(e)))
			wait_for_abort(wait_time)
		try: del monitor
		except: pass
		try: del player
		except: pass
		return kodi_utils.logger('NXTFlix Light', 'TraktMonitor Service Finished')

class UpdateCheck:
	def run(self):
		if kodi_utils.get_property(firstrun_update_prop) == 'true': return
		kodi_utils.logger('NXTFlix Light', 'UpdateCheck Service Starting')
		...
		return kodi_utils.logger('NXTFlix Light', 'UpdateCheck Service Finished')

class WidgetRefresher:
	def run(self):
		kodi_utils.logger('NXTFlix Light', 'WidgetRefresher Service Starting')
		from time import time
		monitor, player = kodi_utils.kodi_monitor(), kodi_utils.kodi_player()
		...
		while not monitor.abortRequested():
			try:
				wait_for_abort(10)
				offset = int(get_setting('nxtflixlt.widget_refresh_timer', '60'))
				if offset != self.offset:
					self.set_next_refresh(time())
					continue
				if self.condition_check(): continue
				if self.next_refresh < time():
					kodi_utils.logger('NXTFlix Light', 'WidgetRefresher Service - Widgets Refreshed')
		...
		return kodi_utils.logger('NXTFlix Light', 'WidgetRefresher Service Finished')
	...
	def condition_check(self):
		...
		if kodi_utils.get_property('nxtflixlt.window_loaded') == 'true': return True 
		try:
			window_stack = json.loads(kodi_utils.get_property('nxtflixlt.window_stack'))
			if window_stack or window_stack == []: return True
		except: pass
		return False

	def set_next_refresh(self, _time):
		self.offset = int(get_setting('nxtflixlt.widget_refresh_timer', '60'))
...
class AutoStart:
	def run(self):
		kodi_utils.logger('NXTFlix Light', 'AutoStart Service Starting')
		from modules.settings import auto_start_nxtflixlt
		if auto_start_nxtflixlt(): kodi_utils.run_addon()
		return kodi_utils.logger('NXTFlix Light', 'AutoStart Service Finished')

class AddonXMLCheck:
	def run(self):
		kodi_utils.logger('NXTFlix Light', 'AddonXMLCheck Service Starting')
		from xml.dom.minidom import parse as mdParse
		self.addon_xml = kodi_utils.translate_path('special://home/addons/plugin.video.nxtflixlt/addon.xml')
		...
		return kodi_utils.logger('NXTFlix Light', 'AddonXMLCheck Service Finished')

	def check_property(self, setting, tag_name):
		current_addon_setting = get_setting('nxtflixlt.%s' % setting, None)
	...
	def change_xml_file(self):
		...
		kodi_utils.logger('NXTFlix Light', 'AddonXMLCheck Service - Change Detected. Restarting Addons')
	...
	def reassign_addon_icon(self):
		...
		addon_icon_choice({'set_icon': get_setting('addon_icon_choice_name', 'nxtflixlt_icon_01.png')})

class nxtflixltMonitor(Monitor):
	def __init__ (self):
	...
	def onNotification(self, sender, method, data):
		if method in ('GUI.OnScreensaverActivated', 'System.OnSleep'):
			...
			kodi_utils.logger('OnNotificationActions', 'PAUSING NXTFlix Light Services Due to Device Sleep')
		elif method in ('GUI.OnScreensaverDeactivated', 'System.OnWake'):
			kodi_utils.clear_property(pause_services_prop)
			kodi_utils.logger('OnNotificationActions', 'UNPAUSING NXTFlix Light Services Due to Device Awake')

kodi_utils.logger('NXTFlix Light', 'Main Monitor Service Starting')
nxtflixltMonitor().waitForAbort()
kodi_utils.logger('NXTFlix Light', 'Main Monitor Service Finished')
