...
def addon_fanart():
	return (
		get_property('nxtflixlt.addon_fanart')
		or 'special://home/addons/plugin.video.nxtflixlt/resources/media/fanart.jpg'
	)
...
def resolve_list_icon(icon, default_name='folder'):
	...
	if icon_norm.startswith('special://') or 'plugin.video.nxtflixlt/resources/media/' in icon_norm:
...
def external_scraper_settings():
	try:
		external = get_property('nxtflixlt.external_scraper.module')
...
def player_check(mode, params):
	...
	if mode == 'playback.%s' % playback_key():
		...
		from modules.player import NXTFlixtLightPlayer
		NXTFlixtLightPlayer().run(params.get('url', None), params.get('obj', None))
