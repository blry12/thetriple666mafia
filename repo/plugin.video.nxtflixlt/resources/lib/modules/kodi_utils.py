...
def addon_info(info):
	return xbmcaddon.Addon('plugin.video.nxtflixlt').getAddonInfo(info)

def addon_version():
	return get_property('nxtflixlt.addon_version') or addon_info('version')

def addon_path():
	return get_property('nxtflixlt.addon_path') or addon_info('path')

def addon_profile():
	return get_property('nxtflixlt.addon_profile') or translate_path(addon_info('profile'))

def addon_icon():
	return get_property('nxtflixlt.addon_icon') or translate_path(addon_info('icon'))

def addon_icon_mini():
	return get_property('nxtflixlt.addon_icon_mini') or os.path.join(addon_info('path'), 'resources', 'media', 'addon_icons', 'minis',
														os.path.basename(translate_path(addon_info('icon'))))

def addon_fanart():
	return get_property('nxtflixlt.addon_fanart') or translate_path(addon_info('fanart'))
...
def get_icon(image_name, image_folder='icons', image_type='png'):
	...
	return 'https://raw.githubusercontent.com/%s/%s/main/packages/media/%s/%s.%s' \
			% (get_property('nxtflixlt.update.username'), get_property('nxtflixlt.update.location'), image_folder, image_name, image_type)

def get_addon_fanart():
	return get_property('nxtflixlt.default_addon_fanart') or addon_fanart()

def build_url(url_params):
	return 'plugin://plugin.video.nxtflixlt/?%s' % urlencode(url_params)
...
def set_view_mode(view_type, content='files', is_external=None):
	if not get_property('nxtflixlt.use_viewtypes') == 'true': return
	if is_external == None: is_external = external()
	if is_external: return
	view_id = get_property('nxtflixlt.%s' % view_type) or None
...
def addon(addon_id='plugin.video.nxtflixlt'):
...
def run_addon(addon='plugin.video.nxtflixlt', block=False):
	return execute_builtin('RunAddon(%s)' % addon, block)

def external():
	return 'nxtflixlt' not in get_infolabel('Container.PluginName')
...
def refresh_widgets():
	...
	kodi_refresh()
	if get_setting('nxtflixlt.widget_refresh_notification', 'true') == 'true': notification('Widgets Refreshed', 2500)
...
def disable_enable_addon(addon_name='plugin.video.nxtflixlt'):
	import json
	try:
...
def update_kodi_addons_db(addon_name='plugin.video.nxtflixlt'):
	import time
...
def player_check(mode, params):
	...
	if mode == 'playback.%s' % playback_key():
		...
		from modules.player import nxtflixltPlayer
		nxtflixltPlayer().run(params.get('url', None), params.get('obj', None))
...
def volume_checker():
	# 0% == -60db, 100% == 0db
	try:
		if get_property('nxtflixlt.playback.volumecheck_enabled') == 'false' or get_visibility('Player.Muted'): return
		from modules.utils import string_alphanum_to_num
		max_volume = min(int(get_property('nxtflixlt.playback.volumecheck_percent') or '50'), 100)
...
def get_all_icons():
	...
	username, location = get_property('nxtflixlt.update.username'), get_property('nxtflixlt.update.location')
